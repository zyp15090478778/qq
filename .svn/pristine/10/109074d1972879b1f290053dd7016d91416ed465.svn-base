...
function validateAdminId(adminid){
	if(adminid != "" ){
		document.getElementById("adminIdMsg").innerHTML = "<img src=\""
			+ "/images/right.gif\">"
			+ "<font color=\"green\">帐号可用！</font>" ;
		return true ;
	}else{
		document.getElementById("adminIdMsg").innerHTML = "<img src=\""
			+ "/images/wrong.gif\">"
			+ "<font color=\"red\">帐号不能为空！</font>" ;
		return false ;
	}
}
...
function validateForm() {
	return validateAdminId(document.getElementById("admin.adminid").value) &&
	validatePassword(document.getElementById("admin.password").value) &&
	flag ;
}